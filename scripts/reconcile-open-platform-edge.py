...
def main() -> None:
    ...
    if args.check:
        ...
        if not matches or not includes(matches[0], desired):
            print("Open platform EdgeOne rule is missing or differs from the versioned configuration.", file=sys.stderr)
            sys.exit(1)
        print("Open platform EdgeOne rule verified: enabled, no cache, authenticated origin, real client IP.")
        return
    if not args.apply:
